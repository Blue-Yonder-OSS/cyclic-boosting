...
class TestNeutralize2DMetaSmoother(unittest.TestCase):
    ...
    def test_no_effect_on_neutralized_values(self):
        X = np.c_[
    ...
    def test_same_result_as_with_neutralized_values(self):
        X = np.c_[
            np.repeat(np.arange(4), 4) * 1.0,
            np.tile(np.arange(4), 4) * 1.0,
            np.ones(16),
...
class TestWeightedMeanSmoother(unittest.TestCase):
    def test_weighted_mean_multidim(self):
        X = np.c_[
            np.repeat(np.arange(4), 4) * 1.0,
            np.tile(np.arange(4), 4) * 1.0,
            np.ones(16),
...
class TestGroupBySmoother(unittest.TestCase):
    def test_gb_onedim(self):
        X = np.c_[
            [0, 0, 1, 1],
            [10, 10, 1, 1],
        ]
    ...
    def test_gb_multi_index_key_error(self):
        """
        Test to ensure that Pandas does not throw a "Too many indexer" error
    ...
    def test_gb_other_keys_in_fit_than_predict(self):
        """
        Test to ensure that Pandas does not throw a "Too many indexer" error
        instead of the expected KeyError.
        """
    ...
    def test_groupby_smoother_cb_weights(self):
        xone = np.ones(4)
        weights = [0, 0, 0, 1]
    ...
    def test_groupby_smoother_cb_weights_all_zero(self):
        xone = np.ones(4)
        weights = [0, 0, 0, 0]
