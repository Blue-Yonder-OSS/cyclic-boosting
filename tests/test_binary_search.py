...
class TestBinarySearch(unittest.TestCase):
    def test_le(self):
        ...
        n = len(z)
        self.assertEqual(binary_search.le(z, (0.83 + 1) / 2, 0), 8)

        z = np.concatenate([[0.52, 0.82], np.linspace(0.83, 1, 18)])

        self.assertEqual(binary_search.le(z, (0.83 + 1) / 2, 0), 2 + 8)
        self.assertEqual(binary_search.le(z, 1.0, 0), len(z) - 1)

    def test_le_interp(self):
        z = np.array([3.0, 10.0, 20.0, 25.0])
...
class TestAdoptTestsOfCalculateIndexArray(unittest.TestCase):
    def test_calculate_index_array(self):
    ...
    def test_calculate_index_array_boolean_index(self):
    ...
    def test_calculate_index_array_order(self):
