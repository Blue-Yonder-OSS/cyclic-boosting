...
class TestSeasonalSmoother(unittest.TestCase):
    def test_mc_first_order(self):
    ...
    def test_force_offset_to_zero(self):
    ...
    def test_warning_custom_function_and_order(self):
        my_cff = lambda x: x
    ...
    def test_custom_choice_function(self):
...
class TestSeasonalLassoSmoother(unittest.TestCase):
...
class TestOrthogonalPolynomialSmoother(unittest.TestCase):
    def test_raise_not_fitted_exception(self):
        n = 10
...
class TestPolynomialSmoother(unittest.TestCase):
    def test_polynomial_smoother(self):
        np.random.seed(10)
        n = 1000
        x = np.linspace(-5, 5, n)
...
class TestLinearSmoother(unittest.TestCase):
    def get_data(self, p0=-3, p1=2):
        np.random.seed(10)
        n = 100
    ...
    def test_linear_fit(self):
        X, y, p0, p1 = self.get_data()
        smoother = smoothing.onedim.LinearSmoother(fallback_when_negative_slope=False)
        smoother.fit(X, y)
        np.testing.assert_allclose(smoother.coefficients_, [p0, p1], atol=0.1, rtol=0.1)

    def test_fallback_when_negative_slope(self):
...
class TestWeightedMeanSmoother(unittest.TestCase):
    def test_crosstest_weighted_mean_smoother(self):
...
class TestLSQUnivariateSpline(unittest.TestCase):
    """tests for :class:`nbpy.smoothing.onedim.LSQUnivariateSpline`"""

    def test_lsq_univariate_spline_close(self):
        np.random.seed(10)
        n = 200
        t = [-1, 0, 1]
...
class TestIsotonicRegressor(unittest.TestCase):
    """tests for :class:`nbpy.smoothing.onedim.IsotonicRegressor`"""

    def test_isotonic_regressor(self):
        np.random.seed(10)
        n = 200
        sigma = 1.0 / 10
    ...
    def test_isotonic_regressor_empty_bins(self):
        n = 200
        sigma = 1.0 / 10
        weights = np.ones(n)
