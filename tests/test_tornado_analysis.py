...
def prepare_time_series_dataset() -> tuple:
    ...
    daily_df = pd.DataFrame(
        {
            "date": ts,
            seasonal: daily_seasonality,
            up_linear: linear,
            down: down_monotonicity
        }
        )
...
def test_analysis_modules(prepare_time_series_dataset) -> tuple:
    ...
    for k in desired.keys():
        if report[k] != desired[k]:
            alart = f"{func[k]}'s result is {report[k]}, expect {desired[k]}"\
                    " with daily time series data"
            assert False, alart
